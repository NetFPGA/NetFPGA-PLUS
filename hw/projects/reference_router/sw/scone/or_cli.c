...
void cli_help(router_state* rs, cli_request* req) {
	...
	usage = "\thw iface add [eth0 eth1] [mac adress]\n";
	send_to_socket(req->sockfd, usage, strlen(usage));

	usage = "\thw iface del [eth0 eth1]\n";
	send_to_socket(req->sockfd, usage, strlen(usage));
	...
	usage = "\thw iface [eth0 eth1] up\n";
	send_to_socket(req->sockfd, usage, strlen(usage));

	usage = "\thw iface [eth0 eth1] down\n";
	send_to_socket(req->sockfd, usage, strlen(usage));
...
void cli_hw_help(router_state *rs, cli_request* req) {
	...
	usage = "\thw iface add [eth0 eth1] [mac addr]\n";
	send_to_socket(req->sockfd, usage, strlen(usage));

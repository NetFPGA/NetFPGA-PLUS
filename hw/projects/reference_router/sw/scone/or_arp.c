...
void write_arp_cache_to_hw(router_state* rs) {
	...
	node *cur = rs->arp_cache;
	while((cur != NULL) && (i < NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_WIDTH)) {
		arp_cache_entry* entry = (arp_cache_entry *)cur->data;
	...
	cur = rs->arp_cache;
	while(i < NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_WIDTH) {
...
void write_arp_cache_entry_to_hw(router_state* rs, arp_cache_entry *entry, int row) {
	...
	if(entry != NULL) {
		...
		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_HI,  mac_hi);
		...
		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_A_LOW, mac_lo);

		/* write the next hop ip data */
		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTWRDATA_B_LOW, ntohl(entry->ip.s_addr));
	...
	/* set the row */
	writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_ADDRESS | row);
  	writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 1);
}
...
void cli_nuke_hw_arp_cache_entry(router_state *rs, cli_request *req) {
	...
	if(rs->is_netfpga) {
		...
		if(row > (NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_WIDTH - 1)) {
...
void cli_hw_arp_cache_misses(router_state *rs, cli_request *req) {
	...
	if(rs->is_netfpga) {
		...
		readReg(rs->netfpga_regs, NFPLUS_OUTPUT_PORT_LOOKUP_0_PKT_SENT_TO_CPU_ARP_MISS_CNTR, &misses);
...
void cli_hw_num_pckts_fwd(router_state *rs, cli_request *req) {
	...
	if(rs->is_netfpga) {
		...
		readReg(rs->netfpga_regs, NFPLUS_OUTPUT_PORT_LOOKUP_0_PKT_FORWARDED_CNTR, &number);

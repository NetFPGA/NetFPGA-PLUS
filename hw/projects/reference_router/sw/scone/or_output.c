...
void sprint_hw_rtable(router_state *rs, char **buffer, unsigned int *len) {
	...
	char *buf = (char *)calloc(strlen(HW_RTABLE_COL) + NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_LPM_TCAM_WIDTH*HW_RTABLE_ENTRY_TO_STRING_LEN, sizeof(uint8_t));
	unsigned int total_len = 0;
	...
	for(i=0; i < NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_LPM_TCAM_WIDTH; i++) {
		...
		/* write the row number */
    		...
    		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 0x11);

		/* read the four-tuple (ip, gw, mask, iface) from the hw registers */
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_A_HI, &ip.s_addr);
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_HI, &mask.s_addr);
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_A_LOW, &gw.s_addr);
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_LOW, &port);
...
void sprint_hw_arp_cache(router_state *rs, char **buf, unsigned int *len) {
	...
	char *buffer = calloc(strlen(HW_ARP_CACHE_COL) + NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_WIDTH*HW_ARP_CACHE_ENTRY_TO_STRING_LEN, sizeof(char));
	unsigned int total_len = 0;

	COPY_STRING(buffer, total_len, HW_ARP_CACHE_COL);

	for(i=0; i < NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_WIDTH; i++) {

		/* write the row number */
		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_IP_ARP_CAM_ADDRESS | i);
		writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 0x11);

		/* read the four-touple (mac hi, mac lo, gw, num of misses) */
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_LOW, &gw.s_addr);
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_A_HI, &mac_hi);
		readReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_A_LOW, &mac_lo);
...
void sprint_hw_stats(router_state *rs, char **buf, unsigned int *len) {
	...
	unsigned int total_len = 0;
	char* port_names[4] = {"eth0", "eth1", "cpu0", "cpu1"};
	...
	for (i = 0; i < 2; ++i) {
		snprintf(line, HW_STATS_LEN, "%-4s %12.2f PPS %12.2f kB/s %-4s %12.2f PPS %12.2f kB/s\n",
			...
			port_names[i+2], rs->stats_avg[i+2][0], rs->stats_avg[i+2][1]);
		COPY_STRING(buffer, total_len, line);
	}
...
void sprint_hw_local_ip_filter(router_state *rs, char **buf, unsigned int *len) {
	char *buffer = calloc((1+NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_WIDTH)*HW_LOCAL_IP_FILTER_LEN + 1, sizeof(char));
	unsigned int total_len = 0;
	...
	if (rs->is_netfpga) {
		...
		struct in_addr ip;
		for (i = 0; i < NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_WIDTH; ++i) {
			writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTADDRESS, NFPLUS_OUTPUT_PORT_LOOKUP_0_MEM_DEST_IP_CAM_ADDRESS | i);
			writeReg(rs->netfpga_regs,NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTCOMMAND, 0x11);
			readReg(rs->netfpga_regs, NFPLUS_OUTPUT_PORT_LOOKUP_0_INDIRECTREPLY_B_LOW, &ip.s_addr);
